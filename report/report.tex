...
%	Le simulazioni sono state condotte assumendo una frequenza di campionamento $f_{s}=\SI{16}{\kilo\hertz}$.
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}
			...
			\end{axis}
		\end{tikzpicture}
		\caption{\ac{nm} al variare dell'ordine di predizione lineare $P$.}
	...
	Il ritardo ha due contributi, uno dei quali scala con la dimensione del buffer: ogni blocco deve essere riempito per intero prima di essere riprodotto, e acquisito per intero prima di essere consegnato all'elaborazione, quindi il doppio buffering costa due buffer di ritardo.
	L'altro è fisso, e comprende i filtri di conversione di ADC e DAC e le latenze delle periferiche (USB), per un totale dell'ordine di qualche millisecondo. 
	...
	La dimensione del buffer determina inoltre il carico di calcolo: buffer lunghi implicano più latenza ma meno lavoro per la CPU, perché l'elaborazione viene richiamata poche volte al secondo; buffer corti riducono la latenza ma moltiplicano le chiamate, e il carico cresce.
	...
	L'implementazione in C++ su NU-Tech ha messo l'algoritmo di fronte ai vincoli che in simulazione non esistono, quali la latenza del sistema audio e il carico di CPU, che sono diventati i fattori dimensionanti dei buffer e della lunghezza dei filtri.
