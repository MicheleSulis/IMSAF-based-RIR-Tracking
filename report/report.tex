...
\author{Mauro Michele Maria Costantino \and Michele Sulis}
\date{\today}
...
\begin{document}
	...
	L'algoritmo validato in simulazione viene implementato in C++ come plugin per la piattaforma real-time NU-Tech \cite{nutech} e testato real-time in laboratorio.
...
%	Le simulazioni sono state condotte assumendo una frequenza di campionamento $f_{s}=\SI{16}{\kilo\hertz}$.
	...
	Conclusa la fase di simulazione, l'algoritmo è stato riscritto in C++ come plugin per la piattaforma real time NU-Tech \cite{nutech}, mantenendo la stessa struttura del codice MATLAB.
	In questa sezione si descrivono il setup sperimentale e le prove condotte in laboratorio.

	\subsection{Setup}
	Le prove sperimentali sono state condotte nella camera semianecoica “E. Mattei” (Audio DSP Lab). 
	La strumentazione è di seguito riportata:
	\begin{itemize}
	...
	Tutto il sistema lavora a $\SI{44.1}{\kilo\hertz}$ con una quantizzazione a 32 bit.
	
	La disposizione geometrica è quella dello schema in Figura \ref{fig:setup_misure}: ciascun microfono è posto sull'asse dell'altoparlante corrispondente, rivolto verso di esso, a distanza $d$; gli assi delle due coppie sono paralleli e distano tra di loro $a$.
	
	\begin{figure}[H]
		\centering
		\input{diagrams/setup_misure}
		\caption{Disposizione nello spazio della strumentazione: i due microfoni e i due altoparlanti occupano i vertici del rettangolo di base $a$ e lato $d$.}
		\label{fig:setup_misure}
	\end{figure}
	
	Si sono realizzate tre prove in tre configurazioni diverse:
	\begin{table}[H]
		\centering
		...
		\begin{tabular}{|c|c|c|c|c|}
			\hline
			Prova \# & Configurazione & $d$ & $a$ & Fotografia \\
			\hline
			1 & MIMO 2x2 & \SI{153}{\centi\meter} (sx), \SI{143}{\centi\meter} (dx) & \SI{94}{\centi\meter} & Fig. \ref{fig:setup_prova1} \\
			\hline
			2 & SISO 1x1 & \SI{55}{\centi\meter} & -- & Fig. \ref{fig:setup_prova2} \\
			\hline
			3 & MIMO 2x2 & \SI{25}{\centi\meter} & \SI{50}{\centi\meter} & Fig. \ref{fig:setup_prova3} \\
			\hline
		\end{tabular}
	...
	In NU-Tech sono state costruite due configutazioni distinte, salvate come file di setup \texttt{xml}.
	
	La prima, riportata in Figura \ref{fig:setup_misurazione} serve alla misura della RIR di riferimento: un blocco \texttt{FileRead} riproduce lo sweep di eccitazione verso l'altoparlante, mentre un blocco FileWrite registra il segnale microfonico su file.
	A seguire, il segnale ottenuto viene processato in frequenza per ricavare la risposta impulsiva.
		
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{diagrams/setup_misurazione}
		\caption{Setup NU-Tech per la misura della risposta impulsiva.}
		\label{fig:setup_misurazione}
	\end{figure}
	
	La seconda configurazione è quella della stima adattativa, impiegata sia nella variante 1x1 sia nella 2x2; essendo le due strutturalmente identiche, si riporta direttamente la 2x2 (Figura \ref{fig:setup_2x2_ASIO}). Due blocchi \texttt{FileRead} riproducono gli stessi segnali di riferimento usati in simulazione, il plugin \texttt{IMSAF} li riceve insieme ai segnali microfonici, esegue la decorrelazione e l’adattamento in sottobande, e al momento dell’arresto salva su file le RIR stimate in sequenza. In parallelo, due \texttt{FileWrite} registrano i segnali dei microfoni a scopo diagnostico.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{diagrams/setup_2x2_ASIO}
		\caption{Setup NU-Tech per la stima MIMO con 2 altoparlanti e 2 microfoni.}
		\label{fig:setup_2x2_ASIO}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}[c]{0.49\textwidth}
			\centering
			\includegraphics[width=\textwidth]{images/2x2_front_large}
		\end{subfigure}
		\hfill
		\begin{subfigure}[c]{0.49\textwidth}
			\centering
			\includegraphics[width=\textwidth]{images/2x2_side_large}
		\end{subfigure}
		\caption{Setup della prova 1 ($d = \SI{153}{\centi\meter}$ sx, $\SI{143}{\centi\meter}$ dx, $a = \SI{94}{\centi\meter}$): vista frontale e laterale.}
		\label{fig:setup_prova1}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}[c]{0.49\textwidth}
			\centering
			\includegraphics[width=\textwidth]{images/2x2_front_small}
		\end{subfigure}
		\hfill
		\begin{subfigure}[c]{0.49\textwidth}
			\centering
			\includegraphics[width=\textwidth]{images/2x2_side_small}
		\end{subfigure}
		\caption{Setup della prova 2 ($d = \SI{55}{\centi\meter}$): vista frontale e laterale.}
		\label{fig:setup_prova2}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}[c]{0.49\textwidth}
			\centering
			\includegraphics[width=\textwidth]{images/2x2_front_tiny}
		\end{subfigure}
		\hfill
		\begin{subfigure}[c]{0.49\textwidth}
			\centering
			\includegraphics[width=\textwidth]{images/2x2_side_tiny}
		\end{subfigure}
		\caption{Setup della prova 3 ($d = \SI{25}{\centi\meter}$, $a = \SI{50}{\centi\meter}$): vista frontale e laterale.}
		\label{fig:setup_prova3}
	\end{figure}
	
	\newpage

	\subsection{Latenza e dimensionamento dei buffer}\label{ssec:latenza}
	La prova 1 non ha prodotto una stima valida. 
	Il motivo non è algoritmico ma di sistema, e riguarda la latenza della catena audio.

	Il filtro adattativo non identifica la sola risposta della stanza, ma l'intero percorso tra il segnale di riferimento e quello del microfono, che rientra nel programma solo dopo aver attraversato buffer di uscita, convertitori, aria e buffer di ingresso. 
	La stima rappresenta il ritardo introdotto dalla catena con una sequenza iniziale di coefficienti nulli, che occupano la finestra del filtro senza portare informazione. 
	La risposta acustica deve trovare posto nei tap che restano.

	Il ritardo ha due componenti, una scala con la dimensione del buffer: ogni blocco deve essere riempito per intero prima di essere riprodotto, e acquisito per intero prima di essere consegnato all'elaborazione, quindi il doppio buffering costa due buffer di ritardo.
	L'altra è fissa, e comprende i filtri di conversione di ADC e DAC e le latenze delle periferiche (USB), per un totale dell'ordine di qualche millisecondo. 
	Con il buffer ASIO, il massimo consentito di 1024 campioni fa sì che domini la prima componente e il ritardo complessivo supera i 2000 campioni; al minimo di 16 campioni il doppio buffering vale appena 32 campioni e resta la parte fissa, per un ritardo complessivo di circa 220 campioni, ovvero circa \SI{5}{\milli\second}. 
	
	Il valore è ricavato dalle misure: sottraendo il tempo di volo noto alla posizione del picco si ottengono 221 campioni nella prova 3 (Figura \ref{fig:rir_time_2x2_lab}) e 216 nella prova 2 (Figura \ref{fig:rir_time_1x1_lab}), due riscontri indipendenti della stessa latenza. 
	
	La Tabella \ref{tab:latenza_rt} riassume le tre situazioni incontrate.
	...
	Nei primi due casi il picco della risposta cade oltre la finestra di osservazione: riferimento e segnale microfonico appaiono scorrelati e la stima converge a rumore.
	È quanto accaduto nella prova 1, poi abbandonata in favore delle configurazioni ravvicinate. 
	Nelle prove 2 e 3, invece, il centinaio di tap residui è sufficiente in camera semianecoica, dove il campo è dominato dal suono diretto e dalle prime riflessioni.

	La dimensione del buffer determina inoltre il carico di calcolo: buffer lunghi comportan più latenza ma meno lavoro per la CPU, perché l'elaborazione viene richiamata poche volte al secondo; buffer corti riducono la latenza ma moltiplicano le chiamate, e il carico cresce.
	Lavorando a 16 campioni il margine di CPU è quindi minimno, e la lunghezza delle \ac{rir} stimate è stata dimensionata di conseguenza.
	Nel caso \ac{siso} il canale singolo richiede meno calcoli e ha permesso una \ac{rir} di 512 campioni; nel caso \ac{mimo} 2x2, con quattro percorsi da stimare, 512 campioni non erano sostenibili dalla CPU e 256 non bastavano a contenere ritardo e risposta: si è adottato il compromesso di 350 campioni.
	
	\newpage

	\subsection{Risultati}
	\subsubsection{RIR nel tempo}
	Le Figure \ref{fig:rir_time_1x1_lab} e \ref{fig:rir_time_2x2_lab} riportano il confronto nel tempo tra le \ac{rir} misurate via sweep e quelle stimate dall'algoritmo, troncate alla stessa lunghezza e normalizzate al rispettivo massimo.

	Un primo riscontro della bontà delle stime è dato dai ritardi. 
	
	Nella prova 2 il picco al campione 287 corrisponde alla stessa latenza più i 71 campioni dei \SI{55}{\centi\meter}. 
	...
	Nella prova 3 i picchi delle \ac{rir} misurate cadono a 253 e 264 campioni per i percorsi diretti, ovvero la latenza di sistema più i circa 32 campioni di volo dei \SI{25}{\centi\meter}, e a 288 e 294 campioni per gli incrociati, in accordo con la diagonale di circa $\SI{56}{\centi\meter}$.
		...
		\begin{subfigure}[c]{0.49\textwidth}
			...
			\begin{tikzpicture}
				\begin{axis}[
					...
					\addplot[black, line width=1.1pt] table [x=n, y=mic1_L1_mis] {plots/rir_time_2x2_350.dat};
		...
		\begin{subfigure}[c]{0.49\textwidth}
			...
			\begin{tikzpicture}
				\begin{axis}[
					...
					\addplot[black, line width=1.1pt] table [x=n, y=mic1_L2_mis] {plots/rir_time_2x2_350.dat};
					\addlegendentry{Measured}
		...
		\begin{subfigure}[c]{0.49\textwidth}
			...
			\begin{tikzpicture}
				\begin{axis}[
					...
					\addplot[black, line width=1.1pt] table [x=n, y=mic2_L1_mis] {plots/rir_time_2x2_350.dat};
					\addlegendentry{Measured}
		...
		\begin{subfigure}[c]{0.49\textwidth}
			...
			\begin{tikzpicture}
				\begin{axis}[
					...
					\addplot[black, line width=1.1pt] table [x=n, y=mic2_L2_mis] {plots/rir_time_2x2_350.dat};
					\addlegendentry{Measured}
			...
			\end{tikzpicture}
		\end{subfigure}
		\caption{Confronto nel tempo tra \ac{rir} misurate e stimate, prova 3 (2x2).}
		\label{fig:rir_time_2x2_lab}
	\end{figure}
	
	La Figura \ref{fig:rir_amp_2x2_lab} mostra le \ac{rir} misurate della prova 3 senza normalizzazione: i percorsi incrociati giungono ai microfoni con un picco circa settre volte inferiore a quello dei diretti, per la maggiore distanza percorsa e per il fuori asse di microfoni e altoparlanti.
		...
		\begin{subfigure}[c]{0.49\textwidth}
			...
			\begin{tikzpicture}
				\begin{axis}[
					...
					\addplot[matlaborange, line width=1.1pt] table [x=n, y=mic1_L1] {plots/rir_amp_2x2_350.dat};
					\addlegendentry{L1 (diretto)}
					\addplot[matlabblue, line width=1.1pt] table [x=n, y=mic1_L2] {plots/rir_amp_2x2_350.dat};
		...
		\begin{subfigure}[c]{0.49\textwidth}
			...
			\begin{tikzpicture}
				\begin{axis}[
					...
					\addplot[matlaborange, line width=1.1pt] table [x=n, y=mic2_L2] {plots/rir_amp_2x2_350.dat};
					\addlegendentry{L2 (diretto)}
					\addplot[matlabblue, line width=1.1pt] table [x=n, y=mic2_L1] {plots/rir_amp_2x2_350.dat};
					\addlegendentry{L1 (incrociato)}
				\end{axis}
			\end{tikzpicture}
		\end{subfigure}
		\caption{\ac{rir} misurate della prova 3 senza normalizzazione: per ciascun microfono, percorso diretto e incrociato.}
		\label{fig:rir_amp_2x2_lab}
	\end{figure}

	\subsubsection{RIR in frequenza}
	Nel caso \ac{siso} (Figura \ref{fig:rir_freq_1x1_lab}) la stima segue la misura fino a circa \SI{18}{\kilo\hertz}, con scostamenti compresi tra 1 e 2 \si{\decibel}. 
	Oltre tale limite la stima crolla mentre la misura resta pressoché piatta, come atteso in camera semianecoica: i segnali di riferimento usati per l'adattamento non contengono praticamente energia sopra i \SI{18}{\kilo\hertz}, e in quella regione il filtro non riceve informazione con cui adattarsi. Il confronto ad alta frequenza non è quindi indicativo delle prestazioni dell'algoritmo.

	\begin{figure}[H]
		\centering
		\begin{tikzpicture}
			\begin{axis}[
				title={RIR Frequency Response Comparison (1x1)},
				title style={font=\sffamily\bfseries\small},
				xlabel={Frequency (Hz)},
				ylabel={Magnitude (dB)},
				label style={font=\sffamily\small},
				...
				\addplot[black, line width=1.1pt] table [x=f, y=misurata] {plots/rir_freq_1x1_512.dat};
				\addlegendentry{Measured RIR}
				\addplot[matlabblue, line width=0.6pt] table [x=f, y=stimata] {plots/rir_freq_1x1_512.dat};
				\addlegendentry{Estimated RIR}
			\end{axis}
		\end{tikzpicture}
		\caption{Confronto in frequenza tra \ac{rir} misurata e stimata, prova 2 (1x1).}
		\label{fig:rir_freq_1x1_lab}
	\end{figure}

	Nel caso \ac{mimo} (Figura \ref{fig:rir_freq_2x2_lab}) il comportamento dipende dal tipo di percorso: i due diretti risultano pressoché sovrapposti alla misura entro lo stesso limite di banda, mentre gli incrociati ne riproducono l'andamento con maggiore varianza.
	Questi ultimi giungono al microfono con ampiezza circa sette volte inferiore a quella dei diretti (Figura \ref{fig:rir_amp_2x2_lab}) e soffrono quindi di un rapporto segnale rumore peggiore.
		...
		\begin{subfigure}[c]{0.49\textwidth}
			...
			\begin{tikzpicture}
				\begin{axis}[
					...
					\addplot[black, line width=1.1pt] table [x=f, y=mic1_L1_mis] {plots/rir_freq_2x2_350.dat};
					\addlegendentry{Measured}
		...
		\begin{subfigure}[c]{0.49\textwidth}
			...
			\begin{tikzpicture}
				\begin{axis}[
					...
					\addplot[black, line width=1.1pt] table [x=f, y=mic1_L2_mis] {plots/rir_freq_2x2_350.dat};
					\addlegendentry{Measured}
		...
		\begin{subfigure}[c]{0.49\textwidth}
			...
			\begin{tikzpicture}
				\begin{axis}[
					...
					\addplot[black, line width=1.1pt] table [x=f, y=mic2_L1_mis] {plots/rir_freq_2x2_350.dat};
					\addlegendentry{Measured}
		...
		\begin{subfigure}[c]{0.49\textwidth}
			...
			\begin{tikzpicture}
				\begin{axis}[
					...
					\addplot[black, line width=1.1pt] table [x=f, y=mic2_L2_mis] {plots/rir_freq_2x2_350.dat};
					\addlegendentry{Measured}
	...
	All'estremo opposto della banda, soto il centinaio di \si{\hertz}, entrambe le curve calano e diventano irregolari: il taglio inferiore degli altoparlanti si colloca attorno ai \SI{66}{\hertz} \cite{genelec8020a} e finestre di 512 e 350 campioni limitano la risoluzione in frequenza a circa \SI{100}{\hertz}, per cui in quella regione il confronto è poco affidabile.

	\newpage
	\section{Conclusioni}
	Il lavoro ha percorso l'intero arco dell'algoritmo \ac{imsaf}, dallo studio teorico alla verifica sperimentale.
	Le simulazioni MATLAB ne hanno confermato il comportamento atteso: la struttura a sottobande accelera la convergenza rispetto al riferimento \ac{nlms}, con un ottimo per $I = 8$, e le tecniche di decorrelazione rendono trattabile il caso \ac{mimo}, pur con il degrado all'aumentare del numero di altoparlanti.

	L'implementazione in C++ su NU-Tech ha messo l'algoritmo di fronte ai vincoli che in simulazione non esistono, la latenza del sistema audio e il carico di CPU, che sono diventati i fattori dimensionanti dei buffer e della lunghezza dei filtri.
	Nelle prove in camera semianecoica le \ac{rir} stimate coincidono con le misure di riferimento sia nei ritardi, entro pochi campioni su tutti i percorsi, sia in frequenza, con scostamenti di 1 o 2 \si{\decibel} nella banda eccitata.
	L'algoritmo si conferma quindi uno stimatore di \ac{rir} efficace anche in condizioni reali, purché latenza e risorse di calcolo vengano dimensionate con cura.

	\section{Sviluppi futuri}
	L'attività svolta lascia aperti alcuni sviluppi naturali, sia sul fronte sperimentale sia su quello implementativo.

	Il primo riguarda il tracking vero e proprio.
	Le prove condotte sono statiche, mentre l'algoritmo nasce per inseguire risposte tempo varianti: sarebbe interessante modificare la geometria a stima avviata, dopo un intervallo di tempo fissato, ad esempio spostando un microfono o introducendo un ostacolo, e misurare il tempo di riconvergenza.

	Il secondo riguarda il banco filtri.
	L'implementazione attuale, con modulazione complessa e filtraggio ripetuti per ciascun ramo, è poco efficiente: la rappresentazione polifase del banco DFT permetterebbe di ottenere lo stesso risultato con un costo di calcolo sensibilmente inferiore.
	Il margine di CPU così recuperato potrebbe essere speso per filtri più lunghi dei 350 campioni della prova 3, per buffer ancora più corti o per configurazioni con più canali.
