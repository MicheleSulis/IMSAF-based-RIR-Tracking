...
%%
%% Preambolo teorico [?, non ci ho pensato prima, non so se è il caso, chiedere alla prof, ma lo lascerei all'ultimo]
%%      Banco DFT 
...
\begin{document}
	...
	Una conseguenza diretta riguarda la metrica di valutazione.
	Nel caso \ac{siso} il \ac{nm} confronta due vettori; nel caso \ac{mimo} si deve invece sintetizzare in un unico scalare l'errore su tutte le $M\times L$ risposte.
	A tale scopo si impiega la \emph{norma di Frobenius}, che estende il concetto di norma euclidea a una matrice considerando la radice della somma dei quadrati di tutti i suoi elementi:
	
	\begin{equation}
		\mathrm{NM}=20\log_{10}
		\frac{\lVert\mathbf{H}-\mathbf{\hat{H}}\rVert_F}{\lVert\mathbf{H}\rVert_F},
		\qquad
		\lVert\mathbf{A}\rVert_F=\sqrt{\sum_{i,j} \lvert a_{ij}\rvert^2}.
	\end{equation}
	
	In questo modo l'intera collezione di RIR (vere e stimate) viene trattata come un unico oggetto, e il rapporto tra l'energia dell'errore e quella delle risposte vere fornisce un'unica misura di accuratezza, espressa in dB.
	...
	Nello scenario realistico, in assenza del blocco \ac{sfc}, un'unica sorgente viene replicata sui diversi canali, che risultano quindi fortemente correlati.
	...
	In queste condizioni il \ac{nm} (Figura \ref{fig:nm_mimo_corr}) raggiunge un valore limite (\emph{floor}) significativamente più alto rispetto al caso ideale (circa \SI{-10}{\decibel}).
	...
	\begin{figure}[H]
		...
		\caption{\ac{nm} nel caso \ac{mimo} con canali correlati e decorrelazione tramite modulazione di fase. Frequenza di campionamento $f_s=\SI{16}{\kilo\hertz}$.}
	...
	Anche il tracciamento delle alte frequenze migliora notevolemente ma continua a soffrire il fatto che i segnali di ingresso, in quanto segnali audio reali, sono tipicamente passa-basso e, per tale ragione, le loro sottobande relative alle frequenze più alte hanno energie molto basse.
	\begin{figure}[H]
		\centering
	...
	%TODO: la prof ci ha chiesto di aumentare le sottobande via via e provare: si potrebbe fare un plot con tikz all'aumentare delle sottobande del NM (16->32->64 e D/2)
	
	%TODO: ci ha detto anche di provare ad aumentare le RIR (tipo un 2x4)
	
	%TODO: ascoltare il risultato (con sound), magari riportare una sottosezione ma sviluppare il codice
