...
\begin{document}
	...
	La struttura implementata è riassunta in Figura \ref{fig:mimo_diagram}, riadattata da \cite{11464373} omettendo la parte \ac{sfc}.
	Il flusso è il seguente: il vettore dei segnali sorgente $\mathbf{x}(n)\in\mathbb{R}^{L}$ ($L$ canali, con $n$ indice temporale a banda piena) viene decomposto in $I$ sottobande e decimato di un fattore $D$ (Sez. \ref{ssec:dft_bank}), ottenendo i segnali di sottobanda $\mathbf{x}_i(k) \in\mathbb{C}^L$ (con $i=1,\dots,I$ e $k$ indice temporale nel dominio decimato); questi vengono decorrelati tramite modulazione di fase (Sez. \ref{ssec:ph_mod}) e risintetizzati nel vettore $\mathbf{s}(n)\in\mathbb{R}^{L}$ dei segnali fisicamente inviati agli altoparlanti.
	...
	In ciascun ramo il segnale viene prima modulato per riportare la sottobanda $i$ in banda base, poi filtrato col prototipo; avendo ora banda circa $1/I$, può essere decimato di un fattore $D$.
	
	La sintesi è la struttura duale: sovracampionamento per inserzione di zeri, filtraggio interpolatore e modulazione inversa per ricollocare ciascuna sottobanda attorno alla propria frequenza centrale.
	
	Si sceglie $D<I$ (banco \emph{sovracampionato}): la banda di guardia che ne deriva fa sì che le code del prototipo si ripieghino, dopo la decimazione in regioni già fortemente attenuate, rendendo trascurabile l'aliasing tra sottobande.
	Con campionamento critico $D=I$ l'aliasing di banco richiederebbe filtri adattativi incrociati tra sottobande adiacenti, che la struttura qui adottata (un filtro per sottobanda) non prevede.
	
	La cascata analisi sintesi non è comunque l'identità esatta: introduce un ritardo di gruppo pari a $V-1$ campioni (somma dei $(V-1)/2$ di ciascun banco), un guadagno di $1/D$ dovuto all'inserzione di zeri e un errore di ricostruzione residuo che dipende dal progetto del filtro prototipo, aspetti che rientreranno sia nella ricostruzione della \ac{rir} sia nel floor del \ac{nm} osservato nelle simulazioni.
	...
	\begin{equation}
		...
		\label{eq:whitened}
	\end{equation}
	ortogonale al passato per costruzione. Rimuovendo la componente prevedibile, $\underline{\mathbf{u}}_i(k)$ è temporalmente meno correlato di $\underline{\mathbf{s}}_i(k)$: con un ordine $P$ adeguato (in pratica $P = 2$ o $3$ sono sufficienti \cite{11464373}) approssima un processo bianco e fornisce una direzione di aggiornamento meglio condizionata.
	...
	L'uscita è la risposta impulsiva dell'intera cascata, dalla quale si ottiene $\hat{h}_{ml}(n)$ compensando il ritardo di gruppo dei due passaggi nel prototipo ($V-1$ campioni complessivi) e il guadagno $1/D$ della catena decimazione interpolazione, e troncando ai $K$ campioni della \ac{rir} vera, così da confrontare stime e risposte reali sulla stessa lunghezza nel calcolo del \ac{nm}.
	...
	La norma di Frobenius sintetizza in un unico scalare l'errore sull'insieme delle risposte: nel caso \ac{mimo} sulle $M \times L$ \ac{rir}, e nel caso \ac{siso} si riduce alla norma euclidea della singola risposta.
	Il \ac{nm} misura così il rapporto tra l'energia dell'errore di stima e quella delle risposte vere: è la metrica che verrà usata in tutte le simulazioni.
	...
	Il degrado alle alte $I$ è legato all'eccitazione delle sottobande. Suddividendo un segnale reale, tipicamente passa-basso, in un numero crescente di sottobande, ciascuna diventa più stretta e riceve una porzione di energia inferiore: le sottobande relative alle alte frequenze risultano scarsamente eccitate e vengono stimate con difficoltà. Inoltre, con $D = I/4$ la decimazione cresce con $I$, riducendo il numero di campioni disponibili per sottobanda e rallentando la convergenza.
