...
int __stdcall PlugIn::LEPlugin_Process(PinType** Input, PinType** Output, LPVOID ExtraInfo)
{
	...
	// 1. Preparazione Buffer Overlap X e Y (Invariato)
	for (int l = 0; l < L; l++) {
	...
	// 2. Analisi di X e Decorrelazione (OTTIMIZZATA CON IPP)
	...
	// 3. Sintesi degli Speaker (RIMOZIONE DI IPPSMOVE TRAMITE DOPPIO BUFFER)
	...
	for (int k_local = 0; k_local < sub_frames; k_local++) {
		...
		for (int i = 0; i < I; i++) {
			...
			// Analisi microfono (OTTIMIZZATA CON IPP)
			for (int m = 0; m < M; m++) {
				Ipp64fc filter_out = { 0.0, 0.0 };
			...
			// Analisi speaker (OTTIMIZZATA CON IPP)
			for (int l = 0; l < L; l++) {
				Ipp64fc filter_out = { 0.0, 0.0 };
				int read_idx = l * (FrameSize + filter_len - 1) + n_full;
			...
			// 5. Decorrelazione IMSAF (SUPER OTTIMIZZATA PER P <= 3 + CONTROLLO STABILITÀ)
			ippsCopy_64fc(s_ij, u_ij, L* Ki);

			if (P > 0) {
				// Calcoliamo una sola volta per iterazione il coniugato di s_ij
				ippsConj_64fc(s_ij, s_ij_conj, L * Ki);
				...
				// --- RISOLUTORE ESPLICITO SENZA LOOP ---
				...
				// Eseguiamo il whitening SOLO SE il sistema era invertibile in modo sicuro!
				if (solver_ok) {
					for (int p = 0; p < P; p++) {
				...
				// Shift e aggiornamento di entrambe le memorie (Normale e Coniugata) avvengono comunque
				ippsMove_64fc(&S_memory[i * P * L * Ki], &S_memory[i * P * L * Ki + L * Ki], (P - 1) * L * Ki);
				ippsCopy_64fc(s_ij, &S_memory[i * P * L * Ki], L * Ki);
			...
			// 6. Aggiornamento Pesi (OTTIMIZZATA CON IPP VETTORIALE)
			double norm_u_val;
			...
			// Calcola il coniugato di u_ij (Necessario per l'update formula)
			...
			for (int m = 0; m < M; m++) {
				...
				// Update Pesi SIMD rapido: H = H + u_ij* * update_factor
